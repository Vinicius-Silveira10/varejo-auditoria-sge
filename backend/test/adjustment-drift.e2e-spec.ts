...
describe('Adjustment Drift E2E (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    const userOperador = await prisma.usuario.create({
      data: {
        nome: 'Operador Req',
        email: `operador-req-${Date.now()}@test.com`,
        senha,
        perfil: 'OPERADOR',
      },
    });
    ...
    const userGestor = await prisma.usuario.create({
      data: {
        nome: 'Gestor Aprov',
        email: `gestor-apr-${Date.now()}@test.com`,
        senha,
        perfil: 'GESTOR',
      },
    });
    ...
    const loginGestor = await request(app.getHttpServer())
      .post('/auth/login')
      .send({ email: userGestor.email, senhaBruta: 'SenhaE2E123' });
    gestorToken = loginGestor.body.accessToken;

    // Criação do Endereço de Origem
    await prisma.endereco.create({
      data: {
        codigo: `E2E-ADDR-${Date.now()}`,
        zona: 'A',
        tipoZona: 'SECO',
        capacidade: 2000,
        ocupado: 1000,
    ...
    const prod = await prisma.produto.create({
      data: {
        sku: `E2E-DRIFT-${Date.now()}`,
        descricao: 'Produto Drift E2E',
        categoria: 'Teste',
        perecivel: false,
        custoMedio: 10.0,
      },
    ...
    const lote = await prisma.lote.create({
      data: {
        numeroLote: `LOTE-DRIFT-${Date.now()}`,
        produtoId: prod.id,
        quantidade: 1000,
      },
    });
  ...
  afterAll(async () => {
    ...
    await prisma.usuario.deleteMany({
      where: { id: { in: [operadorId, gestorId] } },
    });
    await app.close();
  });
  ...
  it('deve aprovar ajuste com GESTOR usando a classificação do momento da requisição, imune ao drift do saldo', async () => {
    ...
    if (!ajusteId)
      throw new Error(
        `ajusteId não encontrado na resposta: ${JSON.stringify(resRequest.body)}`,
      );
    ...
    if (![200, 201].includes(resApprove.status)) {
      throw new Error(
        `Approve falhou com ${resApprove.status}: ${JSON.stringify(resApprove.body)}`,
      );
    }
    const statusAprovacao =
      resApprove.body.statusAprovacao ??
      resApprove.body.aprovado?.statusAprovacao;
