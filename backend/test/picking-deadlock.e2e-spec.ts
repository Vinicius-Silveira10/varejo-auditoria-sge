...
describe('Picking Deadlock Concurrency (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    if (pedidoRes.status !== 201) {
      throw new Error(
        'Falha ao criar pedido: ' + JSON.stringify(pedidoRes.body),
      );
    }
  ...
  it('deve provar o deadlock entre Picking e RegisterMovement no código antigo, e passar no novo', async () => {
    ...
    const originalUpdateDelta =
      PrismaBatchRepository.prototype.updateQuantidadeDelta;
    jest
      .spyOn(PrismaBatchRepository.prototype, 'updateQuantidadeDelta')
    ...
    jest
      .spyOn(PrismaMovementRepository.prototype, 'create')
      .mockImplementation(async function (this: any, data: any) {
        if (
          ...
          console.log(
            '--- MOVEMENT VAI PAUSAR AGORA ANTES DE CHAIN POINTER ---',
          );
    ...
    if (resPick.status === 500 || resMove.status === 500) {
      console.error(
        'DEADLOCK DETECTADO (Um ou ambos os requests falharam com 500):',
        resPick.body,
        resMove.body,
      );
    }
