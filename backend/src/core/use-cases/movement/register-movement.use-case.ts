...
export class RegisterMovementUseCase {
  ...
  async execute(
    ...
    if (data.enderecoDestinoId) {
      const enderecoDestino = await this.addressRepository.findById(
    ...
    if (data.enderecoOrigemId) {
      const enderecoOrigem = await this.addressRepository.findById(
        data.enderecoOrigemId,
      );
    ...
    if (data.tipo === 'SAIDA' || data.tipo === 'EXPEDICAO') {
      ...
      if (data.tipo === 'EXPEDICAO') {
        const lotesDisponiveis =
          await this.batchRepository.findAvailableByProduct(lote.produtoId);
      ...
      if (data.enderecoOrigemId) {
        ...
        if (enderecoOrigem) {
          novaOcupacaoOrigem = Math.max(
            0,
            enderecoOrigem.ocupado - data.quantidade,
          );
        }
      }
