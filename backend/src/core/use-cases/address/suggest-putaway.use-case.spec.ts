...
describe('SuggestPutawayUseCase', () => {
  ...
  it('deve sugerir endereços REFRIGERADO para produto perecível (RN-ARM-003)', async () => {
    ...
    expect(mockAddressRepo.findAvailableByZona).toHaveBeenCalledWith(
      'REFRIGERADO',
    );
  });
  ...
  it('NÃO deve tentar CONGELADO como fallback para perecível sem espaço REFRIGERADO, deve retornar aviso (RN-ARM-003)', async () => {
    mockProductRepo.findById.mockResolvedValue({
      id: 2,
      sku: 'LEITE',
      descricao: 'Leite',
    ...
    expect(result.aviso).toContain('Nenhum endereço REFRIGERADO disponível');
  });
    ...
    mockAddressRepo.findAvailableByZona.mockResolvedValue([
      {
        ...
        ocupado: 90,
        ativo: true,
      }, // Mais ocupado (score de consolidação alto: 90)
      {
        id: 2,
        codigo: 'A-01-01',
        zona: 'A',
        ...
        ocupado: 10,
        ativo: true,
      }, // Menos ocupado (score de consolidação baixo: 10, mas é zona A!)
    ] as any);
