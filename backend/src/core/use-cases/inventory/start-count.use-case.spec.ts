...
describe('StartCountUseCase', () => {
  ...
  beforeEach(() => {
    ...
    mockProductRepo = {
      findById: jest.fn().mockResolvedValue({ id: 10, sku: 'SKU1', curvaAbc: 'A' } as any),
      findAll: jest.fn(),
      create: jest.fn(),
      findBySku: jest.fn(),
  ...
  it('deve iniciar a contagem, bloquear o endereço associado e bloquear o lote', async () => {
    mockBatchRepo.findById.mockResolvedValue({ id: 1, produtoId: 10, quantidade: 100, ativo: true, emInventario: false } as any);
  ...
  it('deve falhar se lote já estiver em inventário', async () => {
    mockBatchRepo.findById.mockResolvedValue({ id: 1, produtoId: 10, quantidade: 100, ativo: true, emInventario: true } as any);
  ...
  it('deve barrar contagem de produto classe B se o inventário anterior tiver menos de 15 dias', async () => {
    mockBatchRepo.findById.mockResolvedValue({ id: 1, produtoId: 10, quantidade: 100, ativo: true, emInventario: false } as any);
    mockProductRepo.findById.mockResolvedValue({ id: 10, sku: 'SKU1', curvaAbc: 'B' } as any);

    const tenDaysAgo = new Date();
    tenDaysAgo.setDate(tenDaysAgo.getDate() - 10);
    mockCountRepo.findLatestFinishedByProduct.mockResolvedValue({ id: 9, criadoEm: tenDaysAgo } as any);

    await expect(useCase.execute({ loteId: 1, usuarioId: 2 }))
      .rejects.toThrow('RN-INV-004: Frequência de inventário para produtos de classe B não respeitada (mínimo 15 dias).');
  });
  ...
  it('deve permitir contagem de produto classe B se o inventário anterior tiver mais de 15 dias', async () => {
    ...
    mockCountRepo.findLatestFinishedByProduct.mockResolvedValue({ id: 9, criadoEm: twentyDaysAgo } as any);
