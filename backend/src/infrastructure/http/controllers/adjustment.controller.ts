...
export class AdjustmentController {
  ...
  @ApiOperation({ summary: 'Solicitar ajuste de estoque (positivo ou negativo)' })
  @ApiBody({ type: RequestAdjustmentBodyDto })
  @ApiResponse({ status: 201, description: 'Solicitação de ajuste criada e aguardando aprovação.' })
  @ApiResponse({ status: 400, description: 'Dados inválidos ou lote não encontrado.' })
  @ApiResponse({ status: 401, description: 'Não autorizado.' })
  async requestAdjustment(@Body() body: RequestAdjustmentBodyDto, @Req() req: any) {
  ...
  @Post('approve')
  @ApiOperation({ summary: 'Aprovar ou rejeitar uma solicitação de ajuste de estoque' })
  @ApiBody({ type: ApproveAdjustmentBodyDto })
  @ApiResponse({ status: 200, description: 'Ajuste processado. Estoque atualizado se aprovado.' })
  @ApiResponse({ status: 400, description: 'Ajuste não encontrado ou já processado.' })
  @ApiResponse({ status: 403, description: 'Perfil sem permissão para aprovar ajustes.' })
  async approveAdjustment(@Body() body: ApproveAdjustmentBodyDto, @Req() req: any) {
