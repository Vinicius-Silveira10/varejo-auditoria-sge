...
export class AddressController {
  ...
  @ApiQuery({
    name: 'threshold',
    required: false,
    description: 'Percentual de ocupação (ex: 0.9 para 90%)',
  })
  ...
  async getCapacityAlerts(@Query('threshold') threshold?: string) {
    const result = await this.getAddressCapacityAlertsUseCase.execute(
  ...
  @ApiResponse({
    status: 400,
    description: 'Código de endereço já existe (RN-ARM-001).',
  })
  ...
  @ApiResponse({
    status: 400,
    description: 'Endereço possui saldo (RN-ARM-002) ou é o último da zona.',
  })
  async disableAddress(@Param('id') id: string) {
    try {
      ...
      if (
        error.message.includes('RN-ARM-002') ||
        error.message.includes('RN-ARM-003')
      ) {
        throw new BadRequestException(error.message);
      }
      throw error;
  ...
  async suggestPutaway(
    ...
    try {
      ...
      if (
        isNaN(dto.produtoId) ||
        isNaN(dto.quantidade) ||
        dto.quantidade <= 0
      ) {
        throw new BadRequestException(
          'produtoId e quantidade devem ser números válidos maiores que zero.',
        );
      }
