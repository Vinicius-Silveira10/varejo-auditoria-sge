...
import {
  IUnitOfWork,
  UnitOfWorkContext,
} from '../../../core/interfaces/repositories/i-unit-of-work';
    ...
    return this.prisma
      .$transaction(async (tx) => {
        ...
        const ctx: UnitOfWorkContext = {
          ...
          logCustoRepository: new PrismaLogCustoRepository(
            txClient,
            this.hashService,
          ),
          adjustmentRepository: new PrismaAdjustmentRepository(txClient),
          movementRepository: new PrismaMovementRepository(
            txClient,
            this.hashService,
          ),
          ...
          inventoryCountRepository: new PrismaInventoryCountRepository(
            txClient,
          ),
          ...
          lockForUpdate: async (entidade: string, id: number) => {
            ...
            await tx.$queryRawUnsafe(
              `SELECT id FROM "${entidade}" WHERE id = $1 FOR UPDATE`,
              id,
            );
          },
        };
