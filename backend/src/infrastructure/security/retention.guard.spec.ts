...
describe('RetentionGuard (RN-REL-003)', () => {
  it('deve bloquear purge de registro com menos de 5 anos', () => {
    ...
    expect(() => RetentionGuard.assertPurgeable(recentDate)).toThrow(
      ForbiddenException,
    );
    expect(() => RetentionGuard.assertPurgeable(recentDate)).toThrow(
      'RN-REL-003',
  ...
  it('deve bloquear purge de registro com 4 anos', () => {
    ...
    expect(() => RetentionGuard.assertPurgeable(fourYearsAgo)).toThrow(
      ForbiddenException,
    );
  });
  ...
  it('deve permitir purge de registro com mais de 5 anos', () => {
    ...
    expect(() => RetentionGuard.assertPurgeable(sixYearsAgo)).not.toThrow();
  });
